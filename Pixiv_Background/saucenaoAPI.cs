...
namespace Pixiv_Background
{
    public class saucenaoAPI
    {
        ...
        private const string api_key = "d800c4204b9b13d9779a2b05fd38d6e67f6d8cf1"; //this is my private api key, limit: 20 requests per 30 seconds, 300 requests per day
        ...
        {
            var ns = new NetStream(RetryTimes: 0);

            var ms = new MemoryStream();
            var boundary = generate_random_boundary();
            ...
            try
            {
                var connection_stream = ns.HttpPost(request_url, (ulong)ms.Length, postContentType: "multipart/form-data; boundary=" + boundary, urlParam: url_param);
        ...
        public static float QueryImage(string path, out Illust illust, out User user)
        {
            ...
            return QueryImage(Image.FromFile(path), out illust, out user);
        }
